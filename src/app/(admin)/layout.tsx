...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { name: "Leads Ledger", href: "/ledger", icon: ClipboardList },
    { name: "Managers", href: "/managers", icon: Users },
    { name: "Forms List", href: "/forms", icon: ClipboardList },
    ...
    <div className="flex h-screen bg-background overflow-hidden font-sans">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto py-6 px-4 flex flex-col gap-1">
          <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2 px-3">Main Menu</div>
          {navItems.map((item) => {
            // For Form Builder, it should only be active on exact match
            // For Forms List, it should be active on /forms, but NOT on /forms/builder
            const isActive = item.href === '/forms' 
              ? pathname === '/forms' 
          ...
          <div className="flex items-center gap-4">
            <div className="relative hidden md:block">
              ...
              <input 
                type="text" 
                placeholder="Search..." 
                className="pl-9 pr-4 py-1.5 text-sm bg-muted border-none rounded-full focus:ring-2 focus:ring-primary focus:bg-surface transition-all w-64"
              />
            </div>
            ...
            <div className="relative">
              ...
              >
                <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-primary to-info flex items-center justify-center text-white font-medium text-sm">
